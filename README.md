...
- `model`: `opus`（デフォルトモデルは Opus 4.8。`/model` コマンドで変更・保存済み）
- `advisorModel`: `opus`（`advisor` ツールは Opus が担当）
- `effortLevel`: `xhigh`

### モデル切り替え・関連コマンド

- `/model` — セッションのデフォルトモデルを変更（現在 Opus 4.8 に設定）
...
- **Opus**: 現在のデフォルトモデル。アーキテクチャ設計、advisor によるレビュー、難易度の高い判断の主力
- **Sonnet**: `/model` やサブエージェントの `model` パラメータで指定。通常の実装・調査タスク向け
...
`~/.claude/settings.json` の `enabledPlugins` により、以下 7 件はすべて有効化済み。
...
- **概要**: 汎用的な "AI 的" デザインを避け、独自性のあるプロダクション品質のフロントエンド UI/UX を生成するスキル（作者: Anthropic）
- **提供形態**: スキル（`SKILL.md` のみ、コマンドなし）。Web コンポーネント・ページ・アプリの構築依頼時に自動発火
- **発火条件**: 「音楽ストリーミングアプリのダッシュボードを作って」「ランディングページを作って」等、フロントエンド構築を含むタスク
- **特徴**:
  - 実装前に明確な美的方向性（ミニマル / マキシマル / レトロフューチャー / エディトリアル等）を選択
  - タイポグラフィ・配色・アニメーション・細部まで作り込んだ実働コード（HTML/CSS/JS, React, Vue 等）を生成
  - 意図性を重視（強度ではなく一貫したコンセプトの徹底）
